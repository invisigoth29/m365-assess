...
function mapControl(control, product, index) {
  ...
  const controlId = control.Control || control.ControlId || control.PolicyId || `CTRL-${index}`;
  const title = control.Requirement || control.ControlName || control.Description || controlId;
  ...
  if (rawResults.Results) {
    ...
    for (const [product, productResults] of Object.entries(rawResults.Results)) {
      if (Array.isArray(productResults)) {
        for (const ctrl of productResults) {
